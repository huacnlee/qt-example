...
DockBase {
    ...
    background: Shape {
        ...
        ShapePath {
            fillGradient: LinearGradient {
                ...
                GradientStop {
                    position: 0
                    color: scope.activeFocus ? focusDarkBackColor : darkBackColor
                }
                GradientStop {
                    position: 1
                    color: scope.activeFocus ? focusLightBackColor : lightBackColor
                }
            }
            dashPattern: [1, 5]
            startX: 5
    ...
    property string name: "Dock Panel"//窗体显示的标题
    property DockTabView tabBar//用于窗体处于tab停靠状态时记录所属tab
    ...
    property color textColor: "#BFBDB6" // 文字颜色
    property color darkBackColor: "#1F2127" // 渐变暗色
    property color lightBackColor: "#1F2127" // 渐变亮色
    property color focusDarkBackColor: "#0D1017" // 焦点渐变暗色
    property color focusLightBackColor: "#0D1017" // 焦点渐变亮色
    ...
    FocusScope {
        ...
        Rectangle {
            ...
            Rectangle {
                ...
                property bool containsDrag: leftDockReceive.containsDrag | rightDockReceive.containsDrag | topDockReceive.containsDrag | bottomDockReceive.containsDrag | centerDockReceive.containsDrag
                DropArea {
                    id: leftDockReceive
                    anchors.left: parent.left
    ...
    function releaseDock() {
        if (root.state === DockBase.DockState.Float) {
            return;
        }
        let state = root.state;
        root.state = DockBase.DockState.Float;
        root.anchors.fill = undefined;
        var tmp;
        var otherItem;
        switch (state) {
        case DockBase.DockState.Split:
            tmp = root.parent.parent.parent.parent;//定位到DockSplit控件根节点
            otherItem = root.parent.otherItem.children[0];//分隔栏必定有兄弟元素
            otherItem.parent = tmp.parent;
            otherItem.state = tmp.state;
    ...
    function dockLeft(target) {
        ...
        let source;
        let dockState;
        if (target.state === DockBase.DockState.Float) {
            var com = Qt.createComponent("Dock.qml").createObject(target.parent, {
                    "x": target.x,
                    "y": target.y,
                    "width": target.width,
                    "height": target.height,
                    "dockHome": target.dockHome
                });
            com.name = it.name + " & " + target.name;
            com.closable = false;
            myParent = com.contentItem;
            source = target;
            dockState = DockBase.DockState.FillDock;
        } else if (target.state === DockBase.DockState.Tab) {
            myParent = target.tabBar.parent;
            source = target.tabBar;
            dockState = target.tabBar.state;
        } else {
        ...
        case DockBase.DockState.Center:
            {
                var com = Qt.createComponent("Dock.qml").createObject(target.parent, {
                        ...
                        "dockHome": target.dockHome,
                        "showDockArea": false,
                        "state": DockBase.DockState.Float
                    });
                com.name = target.name + " & " + it.name;
                com.closable = false;
                var tab = Qt.createComponent("DockTabView.qml").createObject(com.contentItem, {
                        "state": DockBase.DockState.FillDock
                    });
                target.state = DockBase.DockState.Tab;
                theParent = target.parent = tab.contentItem;
                theTab = target.tabBar = tab;
                break;
            }
        case DockBase.DockState.Tab:
            theParent = target.tabBar.contentItem;
            theTab = target.tabBar;
            break;
        default:
            {
                if (target.parent === target.dockHome) {
                ...
                var tabView = Qt.createComponent("DockTabView.qml").createObject(theParent, {
                        "width": target.width,
                        "height": target.height,
                        "state": target.state
                    });
                target.state = DockBase.DockState.Tab;
                theTab = target.tabBar = tabView;
                theParent = target.parent = tabView.contentItem;
            }
        }
        it.parent = theParent;
        it.state = DockBase.DockState.Tab;
        it.tabBar = theTab;
    }

    function splitToWindow() {
        releaseDock();
        var theWindow = Qt.createComponent("DockWindow.qml").createObject(root.dockHome, {
                "width": root.width,
                "height": root.height
            });
        root.state = DockBase.DockState.Fill;
